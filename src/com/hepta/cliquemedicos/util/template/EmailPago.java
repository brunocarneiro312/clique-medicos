...
public class EmailPago extends EmailTemplate {

	/**
	 * Obtém template de email pago
	 * 
	 * @param campos
	 * @return
	 */
	public static String getTemplate(CamposEmailDTO campos) {
		...
		buffer.append("	                                            <h2><strong style=\"color:#006880;\">VALE COMPRA</strong> </h2>	".trim());
		buffer.append("	                                            <h1>${numeroVoucher}</h1>	".trim());
		buffer.append("	                                            <p style=\"font-family: sans-serif; font-size: 18px; mso-height-rule: exactly; line-height: 22px; padding: 10px; text-align: center;\" >Prezado(a) ${nomeBeneficiario}, seu pedido foi confirmado, apresente esse número de Vale no dia da consulta.<br/> Seu pedidos podem ser consultado pelo site em \"MEUS AGENDAMENTOS\".</p>	".trim());
		...
		template = template.replaceAll("\\$\\{numeroVoucher\\}",          campos.getNumeroVoucher());
