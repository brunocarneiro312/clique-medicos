...
public class EmailAlterarSenha extends EmailTemplate {

	/**
	 * 
	 * Obtém o template do email para alteração de senha
	 * 
	 * @param campos
	 * @param token
	 * @return
	...
	public static String getTemplate(CamposEmailDTO campos, String token) {
		...
		buffer.append("												<img src=\"http://www.hepta.com.br/Public/email-cliquemedicos/logoCliqueMedicos_email.png\" width=\"351\" alt=\"Clique Médicos\"  border=\"0\" /> 	".trim());
		buffer.append("											</td> 	".trim());
		buffer.append("										</tr> 	".trim());
		...
		buffer.append("												<h1>Alterar Senha</h1> <p style=\"font-family: sans-serif; font-size: 18px; mso-height-rule: exactly; line-height: 22px; padding: 10px; text-align: center;\" >Recebemos uma solicitação para redefinir sua senha. Para recuperar a sua senha no Clique Médicos, use o link de verificação abaixo</p> 	".trim());
		buffer.append("												 	".trim());
		buffer.append("												 <h2 style=\"text-align: center\"><a style=\"color: #006880\" href=\"${emailAlteracaoSenha}\">${emailAlteracaoSenha}</a></h2> 	".trim());
		buffer.append("											</td> 	".trim());
		buffer.append("										</tr> 	".trim());
		buffer.append("									</table>                        	".trim());
		buffer.append("								</td> 	".trim());
		...
		buffer.append("														<p>Caso não tenha solicitado a alteração da senha, desconsidere esse e-mail.       Atenciosamente, Equipe Clique Médicos</p> 	".trim());
		buffer.append("														<br> 	".trim());
		buffer.append("														<br> 	".trim());
		buffer.append("													</td> 	".trim());
		buffer.append("												</tr> 	".trim());
		...
		template = template.replaceAll("\\$\\{emailAlteracaoSenha\\}", AmbienteUtil.ENDERECO_EMAIL_ALTERACAO_SENHA + token);
