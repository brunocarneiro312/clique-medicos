...
public class EmailCancelado extends EmailTemplate {

	/**
	 * Obtém o template de email cancelado
	 * 
	 * @param campos
	 * @return
	 */
	public static String getTemplate(CamposEmailDTO campos) {
		...
		buffer.append("	                                    <tr>	".trim());
		buffer.append("	                                        <td dir=\"ltr\" valign=\"top\" style=\"font-family: sans-serif; font-size: 15px; mso-height-rule: exactly; line-height: 18px; color: #FFFFFF; padding: 10px; text-align: center;\" class=\"center-on-narrow\">	".trim());
		buffer.append("	                                            <h2><strong style=\"color:#FFFFFF;\">CONSULTA CANCELADA</strong> </h2>	".trim());
		buffer.append("	                                            <h1>Cancelamento de Consulta</h1>	".trim());
		buffer.append("	                                            <p style=\"font-family: sans-serif; font-size: 18px; mso-height-rule: exactly; line-height: 22px; padding: 10px; text-align: center;\" >	".trim());
		buffer.append("												Prezado(a) ${nomeBeneficiario}, sua consulta foi cancelada. Não foi possível confirmar o pagamento. Tente novamente.<br/> Os dados e os estados de seus pedidos podem ser consultado pelo site em \"MEUS AGENDAMENTOS\".</p>	".trim());
		buffer.append("												<p  style=\"text-align: center\"> ${dataCompraEmail}</p> <br/><br/>".trim());
		buffer.append("	                                        </td>	".trim());
		buffer.append("	                                    </tr>	".trim());
		buffer.append("	                                </table>	".trim());
		buffer.append("	                            </td>	".trim());
